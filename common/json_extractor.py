#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""LLM 応答テキストからの JSON 抽出 / 修復ユーティリティ。

LLM はスキーマを指定しても周辺に説明文や ``​ ```json`` ブロックを混ぜて
返すことがあり、`json.loads` がそのままでは通らないケースがそれなりにある。
このモジュールは:

    - ` ```json ... ``` ` ブロックの剥がし
    - 1 番外側の ``{...}`` または ``[...]`` のバランス検出
    - ``json.loads`` の段階的フォールバック

を担当する。``processors/`` と ``api_client`` の両方から使われる。
"""
